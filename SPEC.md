...
- `[settled]` **The docs site lives in this repo, `apps/docs`** (Astro Starlight), deployed to
  GitHub Pages from `main` by `.github/workflows/docs.yml`. It is also the demo: every page runs
  the library live.
...
- `[assumption]` GitHub Pages at `https://<owner>.github.io/<repo>/` (derived from
  `repository.url`) until there is a domain. One-time repo setting: Pages → Source → GitHub
  Actions.
