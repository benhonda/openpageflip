...
Swap the dependency first. `page-flip` becomes the core package, `react-pageflip` becomes the React one (which needs the core package beside it and React 19):

<PackageManagers pkg={corePkg.name} />

<PackageManagers pkg={`${corePkg.name} ${reactPkg.name}`} />

Then the shape of things, which is what the tables don't quite say on their own:
