This is a public repo - no sensitive stuff can be committed. `CONTRIBUTING.md` is the human-facing version of these rules.
...
- `main` is protected by `.github/rulesets/main.json` (applied with `task github:protect`): contributors land through a squash-merged PR with a green `check`; the maintainer and the release GitHub App bypass it.
