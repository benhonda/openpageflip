// @ts-nocheck -- vendored original, checked only by the parity tests
// Vendored verbatim from Nodlik/StPageFlip@ab30ecc src/Flip/FlipCalculation.ts (MIT). Dev-only parity oracle, never published. One edit: type-only imports use `import type` so isolated transforms can load it.
